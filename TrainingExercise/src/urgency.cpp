...
/**O ciclo de vida dos(as) enfermeiros(as), consiste em estar à espera que a fila de triagem
deixe de estar vazia, retirar de lá o paciente mais antigo, atribuir-lhe o código Manchester
de prioridade, colocá-lo na fila de espera de consulta, e voltar ao passo inicial.*/
...
/**O ciclo de vida dos(as) médicos(as), consiste em estar à espera que a fila de espera deixe
de estar vazia, retirar de lá o paciente mais antigo dos mais prioritários, fazer a consulta
(simulada com uma espera de um tempo aleatório), indicar ao paciente que a consulta
está terminada, e voltar ao passo inicial.*/
...
//Quando um paciente entra na urgencia do hospital
...
/**O ciclo de vida de um paciente, consiste simplesmente em entrar
na urgência (patient_goto_urgency) e esperar pelo fim da consulta
(patient_wait_end_of_consultation;.*/
...
void* patient_life(void* argv)
{
   ...
   */
   ...
   patient_goto_urgency(id);
   patient_wait_end_of_consultation(id);
   memset(&(hd->all_patients[id]), 0, sizeof(Patient));
   return NULL;
}
