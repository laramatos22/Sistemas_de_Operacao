...
// ---
static pthread_t* patients_threads;
static pthread_t* nurses_threads;
static pthread_t* doctors_threads;
static pthread_mutex_t* patients_mutex;
static pthread_cond_t* patients_read_cond;
...
void init_simulation(uint32_t np, uint32_t nd, uint32_t nn)
{
   ...
   hd->num_doctors = nd;
   hd->num_nurses = nn;
   patients_threads = new pthread_t[np];
   nurses_threads = new pthread_t[nn];
   doctors_threads = new pthread_t[nd];
   patients_read_cond = new pthread_cond_t[np];
   patients_mutex = new pthread_mutex_t[np];


   for (uint32_t i = 0; i < np; i++)
   {
      patients_read_cond[i] = PTHREAD_COND_INITIALIZER;
...
// ---
void end_simulation() {
   delete[] patients_threads;
   delete[] nurses_threads;
   delete[] doctors_threads;
   delete[] patients_read_cond;
...
void *doctor_life(void *argv)
{
   while (true)
   {
      ...
      // ---
      cond_broadcast(&patients_read_cond[patient]);
      mutex_unlock(&patients_mutex[patient]);
   }
...
int main(int argc, char *argv[])
{
   ...
   for(uint32_t i = 0; i < ndoctors; i++)
   {
      ...
      //patient_life(i);
      thread_create(&doctors_threads[i], NULL, doctor_life, NULL);
   }

   for (uint32_t i = 0; i < nnurses; i++)
   {
      thread_create(&nurses_threads[i], NULL, nurse_life, NULL);
   }
   ...
   for (uint32_t i = 0; i < npatients; i++)
   {
      ...
      *id = i;
      thread_create(&patients_threads[i], NULL, patient_life, (void *)id);
   }

   for (uint32_t i = 0; i < npatients; i++)
   {
      thread_join(patients_threads[i], NULL);
   }
